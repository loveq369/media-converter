...
- (NSInteger)batchConvert:(NSArray *)files toDestination:(NSString *)destination withOptions:(NSDictionary *)options errorString:(NSString **)error
{
    ...
    for (i = 0; i < [files count]; i ++)
    {
	    ...
	    if (userCanceled == NO)
	    {
            ...
            [progressPanel setTask:[NSString stringWithFormat:NSLocalizedString(@"Encoding file %i of %i to '%@'", nil), i + 1, [files count], options[@"Name"]]];
...
- (NSInteger)encodeFileAtPath:(NSString *)path errorString:(NSString **)error
{
    ...
    NSString *fileName = [[[path lastPathComponent] stringByDeletingPathExtension] stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    else
    {
	    if ([options objectForKey:@"-aspect"])
	    {
    	    ...
    	    CGFloat aspectWidth = [[aspectParts objectAtIndex:0] doubleValue];
    	    CGFloat aspectHeight = [[aspectParts objectAtIndex:1] doubleValue];
    ...
    if (!userCanceled)
    {
        [[MCProgressPanel progressPanel] setStatus:[NSLocalizedString(@"Encoding: ", Localized) stringByAppendingString:displayName]];
	    ...
	    for (pass = 0; pass < passes; pass ++)
	    {
    	    ...
    	    if ([vfFilters count] > 0)
    	    {
	    	    ...
	    	    {
    	    	    if (y > 0)
                        inString = [NSString stringWithFormat:@"[out%li]", outCount - 1];
    	    	    ...
    	    	    {
                        outString = [NSString stringWithFormat:@"[out%li]", (long)outCount];
	    	    	    outCount += 1;
    	    	    }
    	    ...
    	    if ([subtitleType isEqualTo:@"hard"])
    	    {
	    	    ...
	    	    if ([filters count] > 0)
	    	    {
                    inString = [NSString stringWithFormat:@"[out%li]", (long)outCount];
    	    	    myString = @"[wm1]";
	    	    }
    	    ...
    	    if (timeString)
	    	    inputTotalTime = [timeString doubleValue];
    	    ...
    	    while([data = [handle availableData] length]) 
    	    {
	    	    ...
	    	    if ([string rangeOfString:@"time="].length > 0)
	    	    {
    	    	    ...
    	    	    if (passes == 2)
                        currentPass = [NSString stringWithFormat: @"pass %li - ", pass + 1];
    	    	    ...
    	    	    if (inputTotalTime > 0.0)
    	    	    {
	    	    	    ...
	    	    	    {
                            MCProgressPanel *progressPanel = [MCProgressPanel progressPanel];
                            [progressPanel setStatusByAddingPercent:[NSString stringWithFormat: @" (%@%.0f%%)", currentPass, percent]];
                            [progressPanel setValue:percent + (double)number * 100];
	    	    	    }
    	    	    }
    	    	    else
    	    	    {
                        [[MCProgressPanel progressPanel] setStatusByAddingPercent:[NSString stringWithFormat:@" (%@?%%)", currentPass]];
    	    	    }
	    	    }
...
{    
    ...
    if ([inputString rangeOfString:@"Video:"].length > 0)
    {
	    ...
	    inputFps = [[fpsArray objectAtIndex:[fpsArray count] - 1] doubleValue];
    ...
    if (hasInputDARValue)
    {
	    ...
	    inputAspect = [[darComponents objectAtIndex:0] doubleValue] / [[darComponents objectAtIndex:1] doubleValue];
    }
    else
    {
	    inputAspect = (CGFloat)inputWidth / (CGFloat)inputHeight;
...
- (NSString *)ffmpegOutputForPath:(NSString *)path
{
    ...
    [MCCommonMethods launchNSTaskAtPath:[MCCommonMethods ffmpegPath] withArguments:arguments outputError:YES outputString:YES output:&string inputPipe:nil predefinedTask:nil];
...
- (BOOL)createMovieWithSubtitlesAtPath:(NSString *)path inputFile:(NSString *)inFile ouputType:(NSString *)type currentOptions:(NSArray *)options withSize:(NSSize)size
{
    ...
    if ([subtitlePaths count] > 0)
    {
	    ...
	    else if ([type isEqualTo:@"hard"])
	    {
    	    ...
    	    if (![[NSFileManager defaultManager] fileExistsAtPath:subtitlePath])
...
- (void)createSubtitleMovieAtPath:(NSString *)path withOptions:(NSArray *)options subtitleFile:(NSString *)file withSize:(NSSize)size
{
    ...
    while ([srtFile rangeOfString:[NSString stringWithFormat:@"<br>%li<br>", (long)i]].length > 0)
    {
	    if (userCanceled)
	    {
    	    break;
        ...
        NSArray *components = [srtFile componentsSeparatedByString:[NSString stringWithFormat:@"<br>%li<br>", (long)i]];
	    
	    NSInteger startIndex = 0;
	    if ([components count] > 1)
    	    startIndex = 1;
	    
        if ([srtFile rangeOfString:[NSString stringWithFormat:@"<br>%li<br>", i + 1]].length > 0)
	    {
            components = [[components objectAtIndex:startIndex] componentsSeparatedByString:[NSString stringWithFormat:@"<br>%li<br>", i + 1]];
    	    startIndex = 0;
	    }
	    else
	    {
	    ...
	    if (timeLimit && ([self secondsFromFormatedString:[times objectAtIndex:0]] > [[options objectForKey:@"-t"] doubleValue]))
    	    break;
        ...
        [[MCProgressPanel progressPanel] setStatusByAddingPercent:percentString];
	    ...
	    ffmpeg = [[NSTask alloc] init];
	    [ffmpeg setLaunchPath:[MCCommonMethods ffmpegPath]];
        [ffmpeg setArguments:[NSArray arrayWithObjects:@"-loop", @"1", @"-f", @"image2", @"-r", [NSString stringWithFormat:@"%0.2f", fps], @"-i", emptyImagePath, @"-vframes", [NSString stringWithFormat:@"%li", (long)emptyDuration], @"-vcodec", @"copy", @"-f", @"avi", @"-", nil]];
	    ...
	    ffmpeg = [[NSTask alloc] init];
	    [ffmpeg setLaunchPath:[MCCommonMethods ffmpegPath]];
        [ffmpeg setArguments:[NSArray arrayWithObjects:@"-loop", @"1", @"-f", @"image2", @"-r", [NSString stringWithFormat:@"%0.2f", fps], @"-i", imagePath, @"-vframes", [NSString stringWithFormat:@"%li", (long)subDuration], @"-vcodec", @"copy", @"-f", @"avi", @"-", nil]];
	    [ffmpeg setStandardError:[NSFileHandle fileHandleWithNullDevice]];
	    [ffmpeg setStandardOutput:outHandle];
	    [ffmpeg launch];
...
- (BOOL)extractSubtitlesFromMKVMovie:(NSString *)inPath ofType:(NSString *)type toPath:(NSString *)outPath
{
    ...
    for (i = 0; i < [trackDictionaries count]; i ++)
    {
	    ...
	    {
            language = [NSString stringWithFormat:@"%@_%li", language, (long)extNumber];
...
{    
    ...
    for (i = 0; i < [subtitles count]; i ++)
    {
	    ...
	    NSString *movieWidth = [NSString stringWithFormat:@"%li", (long)inputWidth];
	    NSString *movieHeight = [NSString stringWithFormat:@"%li", (long)inputHeight];
        ...
        [spumux setArguments:[NSArray arrayWithObjects:@"-s", [NSString stringWithFormat:@"%li", (long)i], xmlPath, nil]];
